          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {activeTab === "financials" && !financials && (
              <div className="glass-card p-12 flex flex-col items-center justify-center text-center space-y-4">
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-electric-500/10 text-electric-400">
                  <svg className="h-7 w-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                </div>
                <h3 className="text-md font-semibold text-white">Financial Data Not Available</h3>
                <p className="text-xs text-gray-500 max-w-sm leading-relaxed">
                  10-year financial statements could not be loaded for <span className="font-mono text-electric-400">{ticker}</span>. This may be because the company is newly added or financial data is being ingested.
                </p>
                <button
                  onClick={() => {
                    const token = localStorage.getItem("access_token");
                    const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
                    fetch(`${apiUrl}/api/v1/companies/${ticker}/financials`, {
                      headers: { Authorization: `Bearer ${token}` }
                    })
